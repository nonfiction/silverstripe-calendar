	...
	public function onBeforeWrite() {
		...
		if (CalendarConfig::subpackage_setting('events', 'enable_allday_events')) {
			...
			if (!$this->ID) {
				if (date("H:i", strtotime($this->StartDateTime))  == '00:00') {
					$this->AllDay = true;
					if ($debug) $this->debugLog('Converted to allday event as the entered time was 00:00');
				}
			}
		}
		...
		if ($this->TimeFrameType == 'Duration') {
			...
			if ($this->StartDateTime != $formatDate) {
				...
				if ($debug) $this->debugLog('Time frame type: Duration: Set end date');
			} else {
				//setting the end date/time to null, as it has automatically been set via javascript
				$this->EndDateTime = null;
				if ($debug) $this->debugLog('Time frame type: Duration: setting the end date/time to null, as it has automatically been set via javascript');
			}
		} else {
			//reset duration
			$this->Duration = '';
